...
class FileRenameTool:
    ...
    def __init__(self, root):
        ...
        self.root.geometry("1200x1000")  # 增加窗口尺寸
    ...
    def create_widgets(self):
        ...
        # --- 步骤 1: 路径选择 ---
        path_frame = ttk.LabelFrame(main_frame, text="步骤 1: 选择路径")
        path_frame.grid(row=0, column=0, columnspan=2, sticky="ew", pady=5)
        path_frame.columnconfigure(1, weight=1)

        ttk.Label(path_frame, text="姓名与文件名对应表 (Excel/CSV):").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self.csv_path_var = tk.StringVar()
        ttk.Entry(path_frame, textvariable=self.csv_path_var).grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        ttk.Button(path_frame, text="浏览...", command=self.select_csv_file).grid(row=0, column=2, padx=5, pady=5)

        ttk.Label(path_frame, text="源文件夹 (待处理文件/夹所在位置):").grid(row=1, column=0, padx=5, pady=5,
                                                                             sticky="w")
        self.source_path_var = tk.StringVar()
        ttk.Entry(path_frame, textvariable=self.source_path_var).grid(row=1, column=1, padx=5, pady=5, sticky="ew")
        ttk.Button(path_frame, text="浏览...", command=self.select_source_folder).grid(row=1, column=2, padx=5, pady=5)

        ttk.Label(path_frame, text="目标文件夹 (重命名后存放位置):").grid(row=2, column=0, padx=5, pady=5, sticky="w")
        self.target_path_var = tk.StringVar()
        ttk.Entry(path_frame, textvariable=self.target_path_var).grid(row=2, column=1, padx=5, pady=5, sticky="ew")
        ttk.Button(path_frame, text="浏览...", command=self.select_target_folder).grid(row=2, column=2, padx=5, pady=5)

        # --- 安全提示 ---
        safety_label = ttk.Label(main_frame, text="▲ 安全提示: 本工具会【复制】文件/夹到目标位置，不会修改您的源文件夹。",
                                 foreground="blue", font=("SimHei", 10, "bold"))
        safety_label.grid(row=1, column=0, columnspan=2, sticky="w", padx=5, pady=(0, 10))

        # --- 步骤 2: 规则配置 (重构) ---
        rules_frame = ttk.LabelFrame(main_frame, text="步骤 2: 配置规则")
        rules_frame.grid(row=2, column=0, columnspan=2, sticky="ew", pady=5)
        rules_frame.columnconfigure(0, weight=1)
        rules_frame.columnconfigure(1, weight=1)

        # 左侧规则区
        left_rules_frame = ttk.Frame(rules_frame, padding=5)
        left_rules_frame.grid(row=0, column=0, sticky="nsew", padx=5)
        left_rules_frame.columnconfigure(1, weight=1)

        # 右侧规则区
        right_rules_frame = ttk.Frame(rules_frame, padding=5)
        right_rules_frame.grid(row=0, column=1, sticky="nsew", padx=5)
        right_rules_frame.columnconfigure(1, weight=1)

        # 对应关系
        map_frame = ttk.LabelFrame(left_rules_frame, text="对应关系", padding=10)
        map_frame.pack(fill="x", expand=True)
        map_frame.columnconfigure(1, weight=1)
        ttk.Label(map_frame, text="姓名列:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self.name_column_var = tk.StringVar()
        self.name_column_combo = ttk.Combobox(map_frame, textvariable=self.name_column_var, state="readonly")
        self.name_column_combo.grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        ttk.Label(map_frame, text="目标名称列:").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        self.filename_column_var = tk.StringVar()
        self.filename_column_combo = ttk.Combobox(map_frame, textvariable=self.filename_column_var, state="readonly")
        self.filename_column_combo.grid(row=1, column=1, padx=5, pady=5, sticky="ew")

        # 匹配规则
        match_frame = ttk.LabelFrame(left_rules_frame, text="匹配规则", padding=10)
        match_frame.pack(fill="x", expand=True, pady=10)
        ttk.Label(match_frame, text="重命名对象:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self.rename_mode_var = tk.StringVar(value="文件")
        ttk.Radiobutton(match_frame, text="文件", variable=self.rename_mode_var, value="文件",
                        command=self._update_folder_options_state).grid(row=0, column=1, sticky="w")
        ttk.Radiobutton(match_frame, text="文件夹", variable=self.rename_mode_var, value="文件夹",
                        command=self._update_folder_options_state).grid(row=0, column=2, sticky="w")
        ttk.Label(match_frame, text="匹配方式:").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        self.match_type_var = tk.StringVar(value="包含")
        ttk.Radiobutton(match_frame, text="包含", variable=self.match_type_var, value="包含").grid(row=1, column=1,
                                                                                                   sticky="w")
        ttk.Radiobutton(match_frame, text="精确匹配", variable=self.match_type_var, value="精确").grid(row=1, column=2,
                                                                                                       sticky="w")

        # 输出选项
        output_frame = ttk.LabelFrame(right_rules_frame, text="输出选项", padding=10)
        output_frame.pack(fill="x", expand=True)
        ttk.Label(output_frame, text="同名文件处理:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self.duplicate_handling_var = tk.StringVar(value="序号")
        ttk.Radiobutton(output_frame, text="添加序号", variable=self.duplicate_handling_var, value="序号").grid(row=0,
                                                                                                                column=1,
                                                                                                                sticky="w")
        ttk.Radiobutton(output_frame, text="跳过", variable=self.duplicate_handling_var, value="跳过").grid(row=0,
                                                                                                            column=2,
                                                                                                            sticky="w")

        self.compress_folder_var = tk.BooleanVar(value=False)
        self.compress_checkbutton = ttk.Checkbutton(output_frame, text="压缩文件夹", variable=self.compress_folder_var)
        self.compress_checkbutton.grid(row=1, column=0, columnspan=2, padx=5, pady=5, sticky="w")
        ...
        self.compress_format_combo = ttk.Combobox(output_frame, textvariable=self.compress_format_var,
                                                  values=["zip", "7z", "rar"], state="readonly", width=8)
        self.compress_format_combo.grid(row=1, column=2, padx=5, pady=5, sticky="w")

        # --- 步骤 3: 操作按钮 ---
        action_frame = ttk.LabelFrame(main_frame, text="步骤 3: 执行操作")
        ...
        self.progress_bar.grid(row=0, column=0, columnspan=5, sticky="ew", padx=5, pady=5)

        # 操作引导
        ttk.Label(action_frame, text="请按 ① -> ② -> ③ 的顺序操作:", foreground="green").grid(row=1, column=0,
                                                                                              sticky='w', padx=5)

        # 按钮
        button_container = ttk.Frame(action_frame)
        button_container.grid(row=2, column=0, columnspan=5, sticky="ew", padx=5, pady=5)
        self.load_button = ttk.Button(button_container, text="① 加载数据并匹配", command=self.load_files)
        self.load_button.pack(side="left", padx=5, fill="x", expand=True)
        self.preview_button = ttk.Button(button_container, text="② 生成重命名预览", command=self.generate_preview)
        self.preview_button.pack(side="left", padx=5, fill="x", expand=True)
        self.execute_button = ttk.Button(button_container, text="③ 执行重命名/压缩", command=self.execute_rename)
        self.execute_button.pack(side="left", padx=5, fill="x", expand=True)
        self.reset_button = ttk.Button(button_container, text="重置所有", command=self.reset_all)  # 新增
        self.reset_button.pack(side="left", padx=15, fill="x", expand=True)
        ttk.Button(button_container, text="查看日志", command=self.show_log).pack(side="left", padx=5, fill="x",
                                                                                  expand=True)
        ttk.Button(button_container, text="退出", command=self.root.quit).pack(side="right", padx=5)

        # --- 预览与日志 (使用 PanedWindow，优化比例) ---
        paned_window = ttk.PanedWindow(main_frame, orient=tk.VERTICAL)
        paned_window.grid(row=4, column=0, columnspan=2, sticky="nsew", pady=5)

        # 预览结果区域 (增加权重)
        result_frame = ttk.LabelFrame(paned_window, text="预览与执行结果")
        paned_window.add(result_frame, weight=4)  # 增加权重从3到4
        ...
        self.tree.column("序号", width=50, anchor="center", minwidth=40)
        self.tree.column("匹配的姓名", width=120, minwidth=100)
        self.tree.column("原始名称", width=280, minwidth=200)  # 增加宽度
        self.tree.column("目标名称", width=280, minwidth=200)  # 增加宽度
        self.tree.column("状态", width=100, anchor="center", minwidth=80)
        ...
        # 日志区域 (减少权重)
        log_frame = ttk.LabelFrame(paned_window, text="操作日志")
        paned_window.add(log_frame, weight=1)  # 保持权重为1
        log_frame.rowconfigure(0, weight=1)
        log_frame.columnconfigure(0, weight=1)
        self.log_text = scrolledtext.ScrolledText(log_frame, state=tk.DISABLED, wrap=tk.WORD, height=6)  # 减少高度
        ...
        # 设置主框架的行列权重
        main_frame.rowconfigure(4, weight=1)  # 设置 PanedWindow 所在行可伸缩
        main_frame.columnconfigure(0, weight=1)

        # 设置PanedWindow的初始分割位置 (预览区域占更多空间)
        self.root.after(100, lambda: paned_window.sashpos(0, 600))  # 设置初始分割位置
    ...
    def update_button_states(self):
        ...
        if self.tree.get_children():
            for item in self.tree.get_children():
                # 注意：values返回的是字符串元组，需要转换为列表或直接索引
