...
class FileRenameTool:
    ...
    def __init__(self, root):
        ...
        self.root.geometry("1200x1000")  # 增加窗口尺寸以适应新布局
    ...
    def create_widgets(self):
        main_frame = ttk.Frame(self.root, padding="8")  # 减少内边距
        main_frame.pack(fill="both", expand=True)

        # --- 步骤 1: 路径选择 (紧凑布局) ---
        path_frame = ttk.LabelFrame(main_frame, text="步骤 1: 选择路径")
        path_frame.grid(row=0, column=0, columnspan=2, sticky="ew", pady=3)  # 减少间距
        path_frame.columnconfigure(1, weight=1)

        ttk.Label(path_frame, text="姓名与文件名对应表 (Excel/CSV):").grid(row=0, column=0, padx=3, pady=2, sticky="w")
        self.csv_path_var = tk.StringVar()
        ttk.Entry(path_frame, textvariable=self.csv_path_var).grid(row=0, column=1, padx=3, pady=2, sticky="ew")
        ttk.Button(path_frame, text="浏览...", command=self.select_csv_file).grid(row=0, column=2, padx=3, pady=2)

        ttk.Label(path_frame, text="源文件夹 (待处理文件/夹所在位置):").grid(row=1, column=0, padx=3, pady=2, sticky="w")
        self.source_path_var = tk.StringVar()
        ttk.Entry(path_frame, textvariable=self.source_path_var).grid(row=1, column=1, padx=3, pady=2, sticky="ew")
        ttk.Button(path_frame, text="浏览...", command=self.select_source_folder).grid(row=1, column=2, padx=3, pady=2)

        ttk.Label(path_frame, text="目标文件夹 (重命名后存放位置):").grid(row=2, column=0, padx=3, pady=2, sticky="w")
        self.target_path_var = tk.StringVar()
        ttk.Entry(path_frame, textvariable=self.target_path_var).grid(row=2, column=1, padx=3, pady=2, sticky="ew")
        ttk.Button(path_frame, text="浏览...", command=self.select_target_folder).grid(row=2, column=2, padx=3, pady=2)

        # --- 安全提示 (紧凑) ---
        safety_label = ttk.Label(main_frame, text="▲ 安全提示: 本工具会【复制】文件/夹到目标位置，不会修改您的源文件夹。",
                                 foreground="blue", font=("SimHei", 9, "bold"))  # 减小字体
        safety_label.grid(row=1, column=0, columnspan=2, sticky="w", padx=5, pady=(0, 5))

        # --- 步骤 2: 规则配置 (水平布局优化) ---
        rules_frame = ttk.LabelFrame(main_frame, text="步骤 2: 配置规则")
        rules_frame.grid(row=2, column=0, columnspan=2, sticky="ew", pady=3)
        rules_frame.columnconfigure(0, weight=1)
        rules_frame.columnconfigure(1, weight=1)

        # 左侧规则区
        left_rules_frame = ttk.Frame(rules_frame, padding=3)
        left_rules_frame.grid(row=0, column=0, sticky="nsew", padx=3)
        left_rules_frame.columnconfigure(1, weight=1)

        # 右侧规则区
        right_rules_frame = ttk.Frame(rules_frame, padding=3)
        right_rules_frame.grid(row=0, column=1, sticky="nsew", padx=3)
        right_rules_frame.columnconfigure(1, weight=1)

        # 对应关系
        map_frame = ttk.LabelFrame(left_rules_frame, text="对应关系", padding=5)
        map_frame.pack(fill="x", expand=True)
        map_frame.columnconfigure(1, weight=1)
        ttk.Label(map_frame, text="姓名列:").grid(row=0, column=0, padx=3, pady=2, sticky="w")
        ...
        self.name_column_combo.grid(row=0, column=1, padx=3, pady=2, sticky="ew")
        ttk.Label(map_frame, text="目标名称列:").grid(row=1, column=0, padx=3, pady=2, sticky="w")
        ...
        self.filename_column_combo.grid(row=1, column=1, padx=3, pady=2, sticky="ew")

        # 匹配规则
        match_frame = ttk.LabelFrame(left_rules_frame, text="匹配规则", padding=5)
        match_frame.pack(fill="x", expand=True, pady=5)
        ttk.Label(match_frame, text="重命名对象:").grid(row=0, column=0, padx=3, pady=2, sticky="w")
        ...
        ttk.Label(match_frame, text="匹配方式:").grid(row=1, column=0, padx=3, pady=2, sticky="w")
        self.match_type_var = tk.StringVar(value="包含")
        ttk.Radiobutton(match_frame, text="包含", variable=self.match_type_var, value="包含").grid(row=1, column=1, sticky="w")
        ttk.Radiobutton(match_frame, text="精确匹配", variable=self.match_type_var, value="精确").grid(row=1, column=2, sticky="w")

        # 输出选项
        output_frame = ttk.LabelFrame(right_rules_frame, text="输出选项", padding=5)
        output_frame.pack(fill="x", expand=True)
        ttk.Label(output_frame, text="同名文件处理:").grid(row=0, column=0, padx=3, pady=2, sticky="w")
        self.duplicate_handling_var = tk.StringVar(value="序号")
        ttk.Radiobutton(output_frame, text="添加序号", variable=self.duplicate_handling_var, value="序号").grid(row=0, column=1, sticky="w")
        ttk.Radiobutton(output_frame, text="跳过", variable=self.duplicate_handling_var, value="跳过").grid(row=0, column=2, sticky="w")
        ...
        self.compress_checkbutton.grid(row=1, column=0, columnspan=2, padx=3, pady=2, sticky="w")
        ...
        self.compress_format_combo.grid(row=1, column=2, padx=3, pady=2, sticky="w")

        # --- 步骤 3: 操作按钮 (紧凑布局) ---
        action_frame = ttk.LabelFrame(main_frame, text="步骤 3: 执行操作")
        action_frame.grid(row=3, column=0, columnspan=2, sticky="ew", pady=3)
        action_frame.columnconfigure(0, weight=1)

        # 进度条
        self.progress_var = tk.DoubleVar()
        self.progress_bar = ttk.Progressbar(action_frame, variable=self.progress_var, maximum=100)
        self.progress_bar.grid(row=0, column=0, columnspan=5, sticky="ew", padx=3, pady=2)

        # 操作引导
        ttk.Label(action_frame, text="请按 ① -> ② -> ③ 的顺序操作:", foreground="green").grid(row=1, column=0, sticky='w', padx=3)

        # 按钮
        button_container = ttk.Frame(action_frame)
        button_container.grid(row=2, column=0, columnspan=5, sticky="ew", padx=3, pady=2)
        self.load_button = ttk.Button(button_container, text="① 加载数据并匹配", command=self.load_files)
        self.load_button.pack(side="left", padx=2, fill="x", expand=True)
        ...
        self.execute_button.pack(side="left", padx=2, fill="x", expand=True)
        self.reset_button = ttk.Button(button_container, text="重置所有", command=self.reset_all)
        self.reset_button.pack(side="left", padx=8, fill="x", expand=True)
        ttk.Button(button_container, text="查看日志", command=self.show_log).pack(side="left", padx=2, fill="x", expand=True)
        ttk.Button(button_container, text="退出", command=self.root.quit).pack(side="right", padx=2)

        # --- 预览与日志 (使用 PanedWindow，优化权重分配) ---
        paned_window = ttk.PanedWindow(main_frame, orient=tk.VERTICAL)
        paned_window.grid(row=4, column=0, columnspan=2, sticky="nsew", pady=3)

        result_frame = ttk.LabelFrame(paned_window, text="预览与执行结果")
        paned_window.add(result_frame, weight=7)  # 大幅增加预览区域权重从3到7
        ...
        self.tree.column("原始名称", width=300)
        self.tree.column("目标名称", width=300)
        ...
        paned_window.add(log_frame, weight=2)  # 减少日志区域权重从1到2
        log_frame.rowconfigure(0, weight=1)
        log_frame.columnconfigure(0, weight=1)
        self.log_text = scrolledtext.ScrolledText(log_frame, state=tk.DISABLED, wrap=tk.WORD, height=6)  # 减少默认高度
